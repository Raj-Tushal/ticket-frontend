...
function HotelList({min,max}) {
  const dispatch = useDispatch()
  const state = useSelector((state)=>state.search)
   const location = useLocation()
  const [destination, setDestination] = useState(location.state.destination);
    const [dates, setDates] = useState(""); // Add state for dates
    const [guests, setGuests] = useState(""); // Add state for number of guests
  
   const {data,loading,error} =  useFetch(`/hotels/byCityName?city=${destination}&min=${min || 0}&max=${max || 999}`)
